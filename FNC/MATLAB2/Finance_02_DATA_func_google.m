% function [stock_data] = Finance_02_DATA_func_google(stock,varargin)
%
% Description: This function gets the historical data of a stock from the
% Google Finance nice website.
%
% Usage:   [stock_data] = Finance_02_DATA_func_google(stock,varargin);
...
while (iarg<nargin-1)
    ...
    if isstr(arg)
        switch arg
            ...
            otherwise
                error('no valid argument supplied. Type ''help Finance_02_DATA_func_google'' for info.');
        end
    end
end
...
if gfstatus && isempty(gfdata(34:end)) == 0
    ...
    fprintf('Unable to retrieve data from Google Finance - ');
    stock_data.date=[];
    stock_data.open=[];
    stock_data.high=[];
    stock_data.low=[];
