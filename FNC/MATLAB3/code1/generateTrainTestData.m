...
yyy=1:1000; yyy=cosd(yyy)*1.2; 
load('rscmn.mat')
% y2 = bollinger(rscmn);
% y3 = bollinger(rshmn);
% y4 = bollinger(rslmn);
% y3 = (rsindex(rscmn, 10) - 50)/50;
...
%cos(pi*rand(1)+pi*rand(1,1000)) + cos(pi*rand(1)+pi*rand(1,1000)) ...
    + sin(pi*rand(1)+pi*rand(1,1000));
%[yyy+(rand(1000,1)'-.5)*3+(rand(1000,1)'-.5)*3];

% [t2t_pca(1:inputLength,1:samplelength)];
...
% end
xxx=1:1000; xxx=sin(xxx)*1.2;
load('Trend.mat')
Trend(980:1000,1)=0;
sampleout(1,:) = Trend'; %xxx';
